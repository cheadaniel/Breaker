...
window.addEventListener('resize', function () {
    ...
    if (client.width > 980) msgArrow.innerHTML = "(flèche gauche/flèche droite du clavier pour bouger)";
...
const Paddle = new paddle(xPaddle, yPaddle, widthPaddle, heightPaddle, dxPaddle, fillColorPaddle, strokeColorPaddle, ctx);
...
document.addEventListener("keydown", function (event) { //on appuie sur une touche 
...
document.addEventListener("keyup", function (event) { //on relache une touche
    if (event.keyCode == 37) { //fleche gauche
        leftArrow = false;
    } else if (event.keyCode == 39) { //fleche droite
        rightArrow = false;
...
function movePaddle() {
    if (rightArrow && Paddle.x + Paddle.width - 0.5 < cvs.width) { //deplace vers la droite si c'est strictement inf a la taille max du canvas
        Paddle.x += Paddle.dx;
    } else if (leftArrow && Paddle.x + 0.5 > 0) { //deplace vers la gauche
        Paddle.x -= Paddle.dx;
    }
}
...
const Ball = new ball(xBall, yBall, radiusBall, speedBall, dxBall, dyBall, fillColorBall, strokeColorBall, ctx)


/*********************** BALL AND WALL COLLISION DETECTION ************************/
function ballWallCollision() {
    if (Ball.x + Ball.radius > cvs.width || Ball.x - Ball.radius < 0) { //regarde où la ball rentre en collision : sur l'axe des x 
    ...
    if (Ball.y - Ball.radius < 0) { // regarde si collision : sur l'axe y est au top du canvas = y=0
        Ball.dy = -Ball.dy; //on retire dy a la ball pour qu'elle change de direction
        WALL_HIT.play(); // Active le son lors de la collision Ball/Wall
    }

    if (Ball.y + Ball.radius > cvs.height) { // regarde si collision : sur l'axe y est au bot du canvas = y=taille du canvas
...
function ballPaddleCollision() { //delimite les bord du paddle et les rends hérmétique p/r a la Ball
    if (Ball.x < Paddle.x + Paddle.width // regarde si la collision se fait entre la droite du paddle et la gauche de la balle
        ...
        && Ball.y > Paddle.y) // regarde si la collision se fait entre le bas de la Ball et haut du Paddle
    {
...
let Brick = new brick(rowBrick, columnBrick, widthBrick, heightBrick, offSetLeftBrick, offSetTopBrick, topMarginBrick, fillColorBrick, strokeColorBrick);


let bricks = []; //liste qui va comprendre l'objet brique créé plus bas

function createBricks() {
    for (let r = 0; r < Brick.row; r++) { // nombre de ligne qu'on veux 
        bricks[r] = [];
        for (let c = 0; c < Brick.column; c++) { // nombre de colonne qu'on veux 
            bricks[r][c] = {
                ...
                y: r * (Brick.offSetTop + Brick.height) + Brick.offSetTop + Brick.topMargin, //en partant du top : 0*(15+20)+15+40 = 55 (la premiere ligne se situe a 55 pixel du top)
...
function drawBricks() {
    for (let r = 0; r < Brick.row; r++) {
        for (let c = 0; c < Brick.column; c++) {
            ...
            if (b.status) { // == true on affiche la brique
...
function ballBrickCollision() {
    for (let r = 0; r < Brick.row; r++) {
        for (let c = 0; c < Brick.column; c++) {
            ...
            if (b.status) { // si la brique est non cassé
                if (Ball.x + Ball.radius > b.x /* si la droite de la ball touche la gauche de la brique */
                    && Ball.x - Ball.radius < b.x + Brick.width /* si la gauche de la ball touche la doite de la brique */
                    && Ball.y + Ball.radius > b.y /* si le bas de la ball touche le haut de la brique */
                    && Ball.y - Ball.radius < b.y + Brick.height /* si le haut de la ball touche le bas de la brique */) {
...
function levelUp() {
    ...
    for (let r = 0; r < Brick.row; r++) { // on va regarder chaque brique
        for (let c = 0; c < Brick.column; c++) {
    ...
    if (isLevelDone) { //si level terminé => on recré un nv level
        WIN.play();

        if (LEVEL >= MAX_LEVEL) { // s'arrete au moment ou on atteint le dernier lvl
            showYouWin();
            GAME_OVER = true
...
function loop() {
    ...
    if (!GAME_OVER) { // si GAME_OVER = true alors on stop la demande d'animation
